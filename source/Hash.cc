...
bool Hash::Append (const char* file)
{
	// Check whether filename is valid
	if (file == nullptr) return false;

	// Attempt to open the file and check state
	fstream data (file, ios::in | ios::binary);
...
void Hash::Append (const uint8* data,
				   uintptr dataLength)
{
...
bool Hash::operator == (uint32 hash) const
{
	return Result == hash;
}
