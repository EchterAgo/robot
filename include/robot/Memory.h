...
public:
  explicit Memory(const Process& p = Process());
...
private:
  uintptr MemRead(uintptr address,
                  void* dataOutput,
                  uintptr length,
                  Flags readFlags);

  uintptr MemWrite(uintptr address,
                   const void* data,
                   uintptr length,
                   Flags writeFlags);

  bool SysRead(uintptr address,
               void* dataOutput,
               uintptr length,
               uintptr* bytes);

  bool SysWrite(uintptr address,
                const void* data,
                uintptr length,
                uintptr* bytes);
